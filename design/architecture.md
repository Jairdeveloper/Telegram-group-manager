...
- La ejecucion usa entrypoints canonicos: `app.api.entrypoint:app` y `app.webhook.entrypoint:app`.
...
- Capa API:
  ...
  - `app/api/entrypoint.py`: composition root (wiring) del runtime de API.
- Capa Webhook:
  - `app/webhook/entrypoint.py`: composition root (wiring) del runtime de webhook.
...
- Capa dominio conversacional:
  ...
  - `chat_service/brain.py` y `chat_service/storage.py` proveen defaults de runtime.
