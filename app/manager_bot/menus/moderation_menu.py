...
def create_antichannel_menu(config: Optional[GroupConfig] = None) -> MenuDefinition:
    ...
    menu.add_row().add_action(f"mod:antichannel:toggle", f"{status} (Toggle)", "🔄")
...
def create_antilink_menu(config: Optional[GroupConfig] = None) -> MenuDefinition:
    ...
    menu.add_row().add_action(f"mod:antilink:toggle", f"{status} (Toggle)", "🔄")
    menu.add_row().add_action("nav:back:mod", "🔙 Volver", "🔙")
...
def create_media_moderation_menu(config: Optional[GroupConfig] = None) -> MenuDefinition:
    ...
    menu.add_row().add_action("mod:media:photos:toggle", "📷 Fotos", "📷")
    menu.add_row().add_action("mod:media:videos:toggle", "🎬 Videos", "🎬")
    menu.add_row().add_action("mod:media:documents:toggle", "📄 Documentos", "📄")
    menu.add_row().add_action("mod:media:stickers:toggle", "😀 Stickers", "😀")
    menu.add_row().add_action("nav:back:mod", "🔙 Volver", "🔙")
...
def create_nightmode_menu(config: Optional[GroupConfig] = None) -> MenuDefinition:
    ...
    menu.add_row().add_action(f"mod:nightmode:toggle", f"{status} (Toggle)", "🔄")
